...
function buildFerdtPdf(account){
 ...
 const writeCell=(v,x0,top,x1,bottom,size=5.35,bold=false,color='#000000')=>{v=String(v??'');whiteTop(x0,top,x1,bottom,.75);const left=x0+3.4,rightEdge=x1-5.2,available=Math.max(4,rightEdge-left),measured=width(v,size,bold),fitted=measured<=available?size:Math.max(4,size*(available/measured)),baseline=(top+bottom)/2+fitted*.34,clipX=left,clipY=H-bottom+1.2,clipW=Math.max(1,rightEdge-left),clipH=Math.max(1,bottom-top-2.4);c+=`q ${clipX.toFixed(2)} ${clipY.toFixed(2)} ${clipW.toFixed(2)} ${clipH.toFixed(2)} re W n\n`;textTop(v,rightEdge,baseline,fitted,bold,'right',color);c+='Q\n'};
 if(ref!=='2026-06'){const rb=tpl.refbox,refLeft=rb[0]-46.9,refRight=rb[2]+2;whiteTop(refLeft-2,rb[1]-1,refRight+2,rb[3]+1,.2);textTop('Referência: '+formatRef(ref),refLeft,rb[3]-.55,9.6,false,'left');const db=tpl.datebox;whiteTop(db[0]-2,db[1]-1,db[2]+4,db[3]+1,.2);textTop(a3EndDate(ref),db[0],db[3]-.45,6.36,false,'left')}
