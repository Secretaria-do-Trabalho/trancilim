...
const CONF_REPORTS=[
  {id:'rel1',label:'Relatório I - Ordens bancárias geradas sem retorno bancário',needsExcel:true,keywords:['relatorio i','ordens bancarias','sem retorno bancario']},
  {id:'rel2',label:'Relatório II - Contas contábeis a regularizar',needsExcel:true,keywords:['relatorio ii','contas contabeis','regularizar']},
  {id:'rel3',label:'Relatório III - Lançamentos de receitas orçamentárias',needsExcel:true,keywords:['relatorio iii','lancamentos','receitas orcamentarias']},
  {id:'rel4',label:'Relatório IV - Prévia da quebra de sequencialidade de pagamentos',needsExcel:true,keywords:['relatorio iv','quebra de sequencialidade','pagamentos']},
  {id:'rel5',label:'Relatório V - Ordens bancárias de pagamentos da folha de pessoal',needsExcel:true,keywords:['relatorio v','ordens bancarias','folha de pessoal']},
  {id:'rel6',label:'Relatório VI - Disponibilidade - Comparativo Contábil x Financeiro',needsExcel:true,keywords:['relatorio vi','disponibilidade','comparativo contabil','financeiro']},
  {id:'rel7',label:'Relatório VII - Saldo dos bens patrimoniais - Comparativo SIAFE x Sistema de Patrimônio',needsExcel:true,keywords:['relatorio vii','bens patrimoniais','siafe','patrimonio']},
  {id:'rel8',label:'Relatório VIII - Consignações intempestivas',needsExcel:true,keywords:['relatorio viii','consignacoes intempestivas'],referenceOffset:-2},
  {id:'rel9',label:'Relatório IX - Suprimento de fundos inconsistentes',needsExcel:true,keywords:['relatorio ix','suprimento de fundos'],referenceOffset:-4},
  {id:'rel10',label:"Relatório X - Comparativo DEA's reconhecidas x DEA's executadas",needsExcel:true,keywords:['relatorio x','comparativo dea','reconhecidas','executadas']},
  {id:'rel11',label:'Relatório XI - Listagem de execução de convênios SICONV',needsExcel:true,keywords:['relatorio xi','execucao de convenios','siconv']},
  {id:'balancete',label:'Balancete Mensal',needsExcel:true,keywords:['balancete','conta contabil','saldo atual']},
  {id:'fiscal',label:'Relatório de Situação Fiscal',needsExcel:false,keywords:['informacoes de apoio para emissao de certidao','diagnostico fiscal']}
];
...
function extractUgCode(text,filename=''){const raw=String(text||''),patterns=[/(?:Unidade\s+Gestora|\bUG)\s*[:_\-–]?\s*(\d{6})/i,/\bUG\s*[:_\-–]\s*(\d{6})/i,/Par[aâ]metros\s*:[\s\S]{0,220}?Unidade\s+Gestora\s*:?\s*(\d{6})/i,/Unidade\s+Gestora[\s\S]{0,350}?(\d{6})/i];for(const p of patterns){const m=raw.match(p);if(m)return m[1]}const fn=String(filename||'').match(/(?:UG|UG_)\s*[:_\-]?\s*(\d{6})/i);if(fn)return fn[1];const first=raw.slice(0,2600);const cnpj=(first.match(/CNPJ(?:\s+do\s+certificado)?\s*:?\s*(\d{2}\.\d{3}\.\d{3})/i)||[])[1];const map={'34.223.378':'560901','49.921.771':'590001','41.604.410':'561001','66.187.382':'590101'};return map[cnpj]||''}
...
function analyzePdf(def,file,text,pages){const ref=confRef(),checks=[],titleOk=titleMatches(def,norm(text),file.name),uc=ugCheck(text,file.name,def),cc=competenceCheck(text,file.name,def);checks.push({category:'dados',status:titleOk?'ok':'error',detail:titleOk?'Relatório correto':'Nome/conteúdo não corresponde ao relatório selecionado'});checks.push({category:'ug',status:uc.status,detail:uc.detail});checks.push({category:'competencia',status:cc.status,detail:cc.detail});const emission=findEmission(text),ed=parseBrDate(emission),end=referenceEnd(ref);let emStatus='warn',emDetail='Data de emissão não localizada';if(ed){const diff=(ed-end)/86400000;emStatus=diff>0&&diff<=62?'ok':diff<=0?'error':'warn';emDetail=emStatus==='ok'?`Emissão ${emission} após o fechamento da competência`:emStatus==='error'?`Emissão ${emission} anterior ou igual ao fim da competência`:`Emissão ${emission} fora da janela usual`}checks.push({category:'emissao',status:emStatus,detail:emDetail});if(def.referenceOffset){const m=text.match(/Data de Refer[eê]ncia\s*:?\s*(\d{2}\/\d{2}\/\d{4})/i),expected=shiftReferenceDate(ref,def.referenceOffset),found=m?.[1]||'';checks.push({category:'competencia',status:found===expected?'ok':found?'warn':'warn',detail:found===expected?`Data de referência específica ${found} correta`:`Data de referência específica encontrada: ${found||'não localizada'}; esperada pelo modelo: ${expected}`})}const dc=dataCheck(def,text);checks.push({category:'dados',status:dc.status,detail:dc.detail});checks.push({category:'dados',status:pages>0?'ok':'error',detail:`PDF com ${pages} página(s)`});return{name:file.name,pages,emission,foundUg:uc.found||'',foundRef:cc.found||'',checks,uploadedAt:new Date().toISOString()}}
  ...
  try{
    for(const def of entries){
      ...
      for(const kind of ['pdf','excel']){
        ...
        try{
          if(kind==='pdf'){const meta=await extractPdfTextMeta(file);entry.pdf=analyzePdf(def,file,meta.text,meta.pages)}
...
async function buildJustificationWordBytes(d){const zip=await JSZip.loadAsync(window.TRANCILIM_JUSTIFICATION_TEMPLATE,{base64:true});let xml=await zip.file('word/document.xml').async('string');const vals={'{{REFERENCE}}':d.reference,'{{PERSON_NAME}}':d.person.name,'{{PERSON_ROLE}}':d.person.role,'{{REGISTRATION}}':d.person.registration,'{{CPF}}':d.person.cpf,'{{UG_BLOCK}}':`${d.ug.code} – ${d.ug.name}`,'{{JUSTIFICATION}}':d.text,'{{LOCATION}}':d.location,'{{DATE_LONG}}':d.dateLong,'{{SIGNATURE_ROLE}}':d.person.signatureRole};for(const [k] of JUSTIFICATION_ITEMS)vals['{{MARK_'+k+'}}']=k===d.item?'X':'';for(const [k,v] of Object.entries(vals))xml=xml.split(k).join(xmlEsc(v));zip.file('word/document.xml',xml);return zip.generateAsync({type:'uint8array',mimeType:'application/vnd.openxmlformats-officedocument.wordprocessingml.document'})}
...
function buildJustificationPdfBytes(d){const W=595.28,H=841.89,img=b64ToBytes(TRANCILIM_ASSETS.logoJpeg),objects=[null],add=o=>(objects.push(o),objects.length-1),catalog=add(''),pagesObj=add(''),fontR=add('<< /Type /Font /Subtype /Type1 /BaseFont /Times-Roman /Encoding /WinAnsiEncoding >>'),fontB=add('<< /Type /Font /Subtype /Type1 /BaseFont /Times-Bold /Encoding /WinAnsiEncoding >>'),imgObj=add({dict:`<< /Type /XObject /Subtype /Image /Width ${TRANCILIM_ASSETS.logoWidth} /Height ${TRANCILIM_ASSETS.logoHeight} /ColorSpace /DeviceRGB /BitsPerComponent 8 /Filter /DCTDecode /Length ${img.length} >>`,data:img});let c='0 G 0 g\n';const text=(v,x,y,size,bold=false,center=false)=>{if(center)x-=pdfTextWidth(v,size,bold)/2;c+=`BT /${bold?'FB':'FR'} ${size} Tf 1 0 0 1 ${x.toFixed(2)} ${y.toFixed(2)} Tm (${pdfStr(v)}) Tj ET\n`};const lineWrap=(v,max,size)=>wrap(v,max*.76,size);const drawWrapped=(v,x,y,max,size,lh,bold=false)=>{const lines=lineWrap(v,max,size);lines.forEach((ln,i)=>text(ln,x,y-i*lh,size,bold));return lines.length};const rich=(runs,x,y,max,size,lh)=>{const words=[];runs.forEach(r=>String(r.text).match(/\S+/g)?.forEach(w=>words.push({w,b:!!r.bold})));const lines=[];let line=[],width=0;for(const it of words){const ww=pdfTextWidth(it.w,size,it.b)*1.28,sp=line.length?pdfTextWidth(' ',size)*1.28:0;if(line.length&&width+sp+ww>max){lines.push(line);line=[];width=0}if(line.length)width+=sp;line.push(it);width+=ww}if(line.length)lines.push(line);lines.forEach((ln,i)=>{c+=`BT 1 0 0 1 ${x} ${y-i*lh} Tm\n`;let f='';ln.forEach((it,j)=>{const nf=it.b?'FB':'FR';if(nf!==f){c+=`/${nf} ${size} Tf\n`;f=nf}c+=`(${pdfStr(it.w+(j<ln.length-1?' ':''))}) Tj\n`});c+='ET\n'});return lines.length};c+=`q 126 0 0 77 ${((W-126)/2).toFixed(2)} 748 cm /Im1 Do Q\n`;text('Anexo VI da IN nº 47/2025',W/2,708,12,true,true);text('Termo de Justificativa para as Inconformidades',W/2,681,11,false,true);text('Referência: '+d.reference,W/2,665,11,false,true);let y=642,x=40,rowH=14,w0=18,w1=497;c+='0.7 w\n';for(const [key,label] of JUSTIFICATION_ITEMS){c+=`${x} ${y-rowH} ${w0} ${rowH} re S\n${x+w0} ${y-rowH} ${w1} ${rowH} re S\n`;if(key===d.item)text('X',x+w0/2,y-10,8,true,true);text(label,x+w0+4,y-10,7.3,false);y-=rowH}text('Obs.: Marcar com “X” uma única opção em cada termo de justificativa.',40,y-11,8);const personY=y-35;rich([{text:'Eu, '},{text:d.person.name,bold:true},{text:', '+d.person.role+', '},{text:d.person.registration,bold:true},{text:', CPF '},{text:d.person.cpf,bold:true},{text:', lotado(a) na '},{text:'UG '+d.ug.code+' – '+d.ug.name,bold:true},{text:', apresento para os devidos fins a(s) justificativa(s) para a(s) inconformidade(s) constatada(s) no documento marcado acima.'}],50,personY,495,9.2,11.2);let justFont=8.4,justLines=lineWrap(d.text,495,justFont);while(justLines.length>12&&justFont>7){justFont-=.3;justLines=lineWrap(d.text,495,justFont)}const boxTop=personY-58,boxH=Math.max(73,Math.min(132,justLines.length*(justFont+2)+25));c+=`50 ${boxTop-boxH} 495 ${boxH} re S\n`;text('Justificativa(s) para a(s) Inconformidade(s) Constatada(s):',W/2,boxTop-11,8,false,true);justLines.slice(0,13).forEach((ln,i)=>text(ln,55,boxTop-25-i*(justFont+2),justFont));const dateY=boxTop-boxH-14;text(`${d.location}, ${d.dateLong}.`,50,dateY,9.5);text(d.person.name,W/2,dateY-38,10,true,true);text(d.person.signatureRole,W/2,dateY-52,10,true,true);const content=add({dict:`<< /Length ${cp1252(c).length} >>`,data:new Uint8Array(cp1252(c))}),page=add(`<< /Type /Page /Parent ${pagesObj} 0 R /MediaBox [0 0 ${W} ${H}] /Resources << /Font << /FR ${fontR} 0 R /FB ${fontB} 0 R >> /XObject << /Im1 ${imgObj} 0 R >> >> /Contents ${content} 0 R >>`);objects[catalog]=`<< /Type /Catalog /Pages ${pagesObj} 0 R >>`;objects[pagesObj]=`<< /Type /Pages /Count 1 /Kids [${page} 0 R] >>`;return makePdf(objects,catalog,pagesObj)}
